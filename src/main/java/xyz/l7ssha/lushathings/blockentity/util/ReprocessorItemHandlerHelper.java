...
public class ReprocessorItemHandlerHelper {
  public static <T extends ReprocessorIOHatch> void pushToNeighbors(
      T blockEntity, IItemHandler neighbor) {
  ...
  public static <T extends ReprocessorIOHatch> void pullFromNeighbors(
      T blockEntity, IItemHandler neighbor) {
    var config = blockEntity.getInventoryConfig();
    if (config == null) {
      return;
    ...
    for (int slot = 0; slot < neighbor.getSlots(); slot++) {
      ...
      if (!stack.isEmpty()) {
        ItemStack remaining =
            ItemHandlerHelper.insertItemStacked(blockEntity.getInputInventory(), stack, true);
        int toMove = stack.getCount() - remaining.getCount();
