...
public class ReprocessorInputOutputMenu extends AbstractContainerMenu {
  ...
  public ReprocessorInputOutputMenu(
      int containerId, Inventory playerInventory, FriendlyByteBuf buf) {
    this(
        containerId,
        playerInventory,
        (ReprocessorInputOutputBlockEntity)
  ...
  public ReprocessorInputOutputMenu(
      ...
      @Nullable ReprocessorInputOutputBlockEntity blockEntity) {
  ...
  private void addGrid(IItemHandler handler, int startX, int startY, boolean allowInsert) {
    ...
    for (int row = 0; row < 3; row++) {
      for (int col = 0; col < 3; col++) {
        ...
        this.addSlot(
            new SlotItemHandler(handler, slotIndex, startX + col * 18, startY + row * 18) {
              @Override
