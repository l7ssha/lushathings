...
public record ReprocessorRecipe(
    NonNullList<SizedIngredient> inputs,
    ItemStack output,
    ItemStack output2,
    int craftingTime,
    int energyCost)
    implements Recipe<ReprocessorRecipeInput> {
  @Override
  ...
  public ItemStack assemble(
      ReprocessorRecipeInput reprocessorRecipeInput, HolderLookup.Provider provider) {
    return output.copy();
  }
  ...
  public static class Serializer implements RecipeSerializer<ReprocessorRecipe> {
    public static final MapCodec<ReprocessorRecipe> CODEC =
            ...
            inst ->
                inst.group(
                        SizedIngredient.CODEC
                            .listOf()
                            .fieldOf("ingredients")
                            .forGetter(ReprocessorRecipe::inputs),
                        ItemStack.CODEC.fieldOf("result").forGetter(ReprocessorRecipe::output),
                        ItemStack.CODEC
                            .optionalFieldOf("result_aux", ItemStack.EMPTY)
                            .forGetter(ReprocessorRecipe::output2),
                        Codec.INT
                            .fieldOf("crafting_time")
                            .forGetter(ReprocessorRecipe::craftingTime),
                        Codec.INT.fieldOf("energy_cost").forGetter(ReprocessorRecipe::energyCost))
                    .apply(
                        inst,
                        (ingredients, result, result2, craftingTime, energyCost) -> {
                          NonNullList<SizedIngredient> inputs = NonNullList.create();
                          inputs.addAll(ingredients);
                          return new ReprocessorRecipe(
                              inputs, result, result2, craftingTime, energyCost);
                        }));

    public static final StreamCodec<RegistryFriendlyByteBuf, ReprocessorRecipe> STREAM_CODEC =
        StreamCodec.composite(
            SizedIngredient.STREAM_CODEC.apply(
